	...
	public class JsonNodePathTest extends BaseTestContainersTest {
				...
				Tuple result = new JPAQuery<JsonNode>(entityManager)
						...
						.select(
								...
								jsonNodeEntity.embed1.get("embed1_attr2.embed2_attr1")
										.asText().eq("embed2_attr1"),
								jsonNodeEntity.embed1
										.get(NEmbed1.embed1.embed1_attr2.embed2_attr1)
										.asText().eq("embed2_attr1"),
								jsonNodeEntity.embed1.get("uuidText").asText()
										.contains(Expressions
												.stringPath(jsonNodeEntity.uuid
														.getMetadata())),
			...
			doInJPA(this::sessionFactory, entityManager -> {
				...
				List<Tuple> result = new JPAQuery<JsonNodeEntity>(entityManager)
						...
						.select(
								...
								embed1.contains(Map.of(NEmbed1.embed1.embed1_attr1,
										"embed1_attr1")),
								embed1.get(NEmbed1.embed1.embed1_attr2.path())
										.contains(Map.of(NEmbed2.embed2.embed2_attr1,
												"embed2_attr1")),
								embed1.contains(
										Map.of(
				...
				List<Integer> result = new JPAQuery<JsonNodeEntity>(entityManager)
						...
						.where(
								jsonNodeEntity.null_1.get("test").isNull()
										.and(jsonNodeEntity.null_2.get("test")
												.isNull()))
						.fetch();
				...
				List<Integer> result = new JPAQuery<JsonNodeEntity>(entityManager)
						...
						.where(
								jsonNodeEntity.not_null.get("test").isNotNull()
										.and(jsonNodeEntity.not_null.get("test")
												.isNull().not()))
						.fetch();
		...
		public void buildJsonObject() {
			doInJPA(this::sessionFactory, entityManager -> {
				ObjectNode result = new JPAQuery<JsonNodeEntity>(entityManager)
						.from(jsonNodeEntity)
						.select(
								JsonExpressions.buildJsonObject(jsonNodeEntity.id.as("id"),
										jsonNodeEntity.id.as("id2")))
						.fetchOne();

				try {
					assertEquals(ImmutableMap.of("id", entity.id.intValue(), "id2", entity.id.intValue()),
							objectMapper.treeToValue(result, Map.class));
		...
		public void buildJsonObject2() {
			doInJPA(this::sessionFactory, entityManager -> {
				ObjectNode result = new JPAQuery<JsonNodeEntity>(entityManager)
						.from(jsonNodeEntity)
						.select(
								JsonExpressions.buildJsonObject(ImmutableMap.of("id",
										jsonNodeEntity.id, "id2", jsonNodeEntity.id)))
						.fetchOne();
		...
		public void testIsEmptyArray(){
			doInJPA(this::sessionFactory, entityManager -> {
				List<Integer> result = new JPAQuery<JsonNodeEntity>(entityManager)
						...
						.select(Expressions.ONE)
						.where(
								jsonNodeEntity.listInt3.isEmptyArray()
										.and(
		...
		public void testDeletePath(){
			doInJPA(this::sessionFactory, entityManager -> {
				...
				try {
					expected = objectMapper.readValue("[\"a\",{}]", JsonNode.class);
